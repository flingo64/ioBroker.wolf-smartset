...
class WolfSmartSet {
        ...
        try {
            if (fs.existsSync(this.authFile)) {
                ...
                this.adapter.log.info(
                    `_removeAuthData(): Auth data cache file ${this.authFile} was removed successfully.`,
                );
            }
        } catch (error) {
            this.adapter.log.warn(
                `_removeAuthData(): Auth data cache file ${this.authFile} could not be removed (${error.message})!`,
            );
        }
    }
    ...
    async _openIdInit() {
        ...
        try {
            this.adapter.log.info(
                `_openIdInit(): trying to discover issuer at ${_API_STRING['WEBSERVER'] + _API_STRING['PATH_OPENID_CFG']}`,
            );
            const openIdIssuer = await Issuer.discover(_API_STRING['WEBSERVER'] + _API_STRING['PATH_OPENID_CFG']);
            this.adapter.log.info(`_openIdInit(): discovered issuer ${openIdIssuer.issuer}`);
            ...
            this.adapter.log.debug(`_openIdInit(): created openIdClient`);
            ...
            this.adapter.log.error(`_openIdInit(): ERROR: ${error.message}`);
            return null;
        }
    ...
    async _getAuthToken() {
        ...
        try {
            ...
            if (!this.openIdClient) {
                throw new Error('_getAuthToken(): _openIdInit() failed!');
            }
            ...
            this.adapter.log.info(`_getAuthToken(): authUrl: ${authUrl}`);
            ...
            this.adapter.log.debug(`_getAuthToken(): requestVerificationToken: ${requestVerificationToken}`);
            ...
            this.adapter.log.debug(`_getAuthToken(): redirectUrl: ${redirectUrl}`);
            ...
            if (!this.openIdStore[params.state]) {
                throw new Error(`Cannot decode response for State ${params.state}.`);
            }
    ...
    async _refreshAuthToken() {
        ...
        try {
            ...
            this.adapter.log.debug(`_refreshAuthToken(): authUrl: ${authUrl}`);
    ...
    async _destroyAuthToken() {
        ...
        // do not call OpenId endSessionUrl: Wolf will return error 429 and prevents new session w/o adapter restart
    ...
    async init() {
        ...
        // init failed
        this.adapter.log.error('Initialization failed!');
        return false;
    }
